...
GraspDetectionNode::GraspDetectionNode(ros::NodeHandle& node) : has_cloud_(true), has_normals_(false),
...
bool GraspDetectionNode::getGraspFromPointCloud(gpd::GetGraspFromPointcloud::Request& req, gpd::GetGraspFromPointcloud::Response& res)
{
    ROS_INFO("getGraspFromPointCloud Service called");
    ...
    // Detect grasps in point cloud.
    std::vector<Grasp> grasps = detectGraspPosesInTopic();
    ROS_INFO("Drawing grasp in Rviz...");
    ...
    has_samples_ = false;
    has_normals_ = false;
    ROS_INFO("getGraspFromPointCloud service OVER");
...
int main(int argc, char** argv)
{
  ...
  // initialize ROS
  ros::init(argc, argv, "grasp_from_pointcloud_server");
  ros::NodeHandle node("~");

  GraspDetectionNode grasp_detection_server(node);
  ros::ServiceServer service = node.advertiseService("grasp_from_pointcloud", &GraspDetectionNode::getGraspFromPointCloud,
                                                     &grasp_detection_server);

  ROS_INFO("grasp_from_pointcloud service started...");

  grasp_detection_server.run();
